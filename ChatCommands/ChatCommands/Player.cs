...
{
	class Player : ChatComm
	{
        public const string format = "/player [player]";
		...
		private void loadPlayerInfo(String playerName)
		{
			...
			wc.DownloadStringAsync(new Uri("http://a.scrollsguide.com/player?fields=rank,rating,name,gold,scrolls&name=" + playerName));
		}
		...
		private void proc(String result, String playerName)
		{
			try
			{
				...
				{
					msg(String.Format("{0} is ranked {1} with a rating of {2}. Gold: {3}. Scrolls: {4}.", ar.data.name, ar.data.rank, ar.data.rating, ar.data.gold, ar.data.scrolls));
				}
				else
				{
